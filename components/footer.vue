<template>
  <footer class="bg-pink">
    <div class="max-w-6xl m-auto p-7">
      <div class="p-always relative">
        <div>
          <h3 class="text-3xl font-semibold mb-4">Vind je mijn werk leuk?</h3>
          <nuxt-link to="#" class="text-2xl"
            >Neem contact met mij op →</nuxt-link
          >
        </div>
        <div class="mt-5">
          ...
          <div class="flex gap-4">
            <div v-for="icon in icons" :key="icon.src">
              ...
              <div
                v-html="icon.src"
                class="w-12 h-12 mt-2 bg-bluebg rounded-full flex justify-center items-center cursor-pointer"
                @click="$router.push(icon.link)"
              ></div>
            </div>
          </div>
        </div>
        <div class="absolute bottom-0 logo 700:hidden">
          <img src="/me.png" alt="" class="w-64" />
        </div>
      </div>
    </div>
  </footer>
